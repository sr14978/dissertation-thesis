...
Information theoretic security is a level of security that is based in information theory.
...
Although this scheme has perfect security, it relies on sharing the same random key (of same size as the message) between the two parties who wish to communicate without leaking it to anyone else.
...
For example, for a key of length 256 bits, it could take upto many times the age of the universe to guess.
Also, the schemes shuffle and mix the ciphertext such that to workout what the key is, by mathematical constraints, many plaintext-ciphertext pairs are needed.

To be a bit more formal, encryption schemes are built from building blocks called block-ciphers.
...
There are bad ways of doing this such as electronic code bock mode (ECB), which suffers from block independence so certain patterns in the plaintext are obvious in the cipher text.
There are also good ways, such as cipher block chaining (CBC) which XORs the previous ciphertext in to the next plaintext to make all ciphertext blocks dependent on all previous plaintext blocks.
This has performance implications since to decode one block you have to decode all previous blocks in the message so there are other modes tailored to that but, this also means the the last block is a calculation that depends on all other blocks. 

This same mode can then be used with a different key to \textit{encrypt} the ciphertext, only keeping the last block. This last block can then be sent with the ciphertext as proof that the ciphertext is not changes while in transit, since recalculation at the destination should result in the same message authentication code (MAC).
...
The way the schemes work is based on a trap door function which is easy to compute in the encryption direction, but to invert for decryption is hard, unless you know the private key.
...
Although, many symmetric encryption scheme produce ciphertexts that are indistinguishable from random bits, asymmetric schemes do not.
...
Therefore, they might have structure to define the separate elements, for instance an ElGamal ciphertext is $(g^y, m \cdot h^y )$ where, $m,g,h \in \mathbb{G}$ which is the group.
...
Most examples of this are just security through obscurity such as invisible ink or hiding text in a image by altering pixel values.
Honey encryption which we introduced in 2014 by Juels et al.~\cite{honey} has a similar tone to steganography, though instead of the resulting encoding appearing like innocuous data, it involves as ciphertext that can be decrypted to the original message or into a innocuous one.
...
\paragraph{In summary,} although, each of these systems is being successfully used today, they each have floors that could be attacked in the future, which inhibit them for being universal solutions. Therefore, as invited by the TOR community, new systems are needed.
...
Therefore, by the definition of $\mathcal{F}^{-1}_{D_N}$, it will generate heuristic values that follow the distribution in normal traffic of $\mathcal{H}$.
