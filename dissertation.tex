...
These may be breaches of privacy right or violations of state laws, but there are other, more universal issues to consider. There is, for instance, a fairly unanimous consensus that child pornography is a bad thing and if access to it can be blocked this would be certainly beneficial to society.
...
One of the cores, proven systems for avoiding censorship is The Onion Router (TOR).
...
If length encoding took in arbitrary length ciphertexts, it would need to include the length of the original into the encoding, such as pre-pending the data with its length as a number of bytes.
...
The filter shows that we only targeted GET requests. This is because HTTP is a very one-sided protocol with most of the content moving from the web server to the client and therefore it is harder to hide messages inside the requests. The response encoding should be more straightforward. GET requests as opposed to POST requests take up a larger proportion of web traffic and so are the obvious choice.

In this experiment, I have fixed all other parameters apart from the path. This is a proof of principle
