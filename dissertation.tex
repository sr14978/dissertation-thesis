...
&&\\
Censor				&:		& A party who is conducting censorship\\
...
I would like to take this opportunity to thank my supervisor -- Bogdan -- for his expertise and I would like to mention, Carl, for his machine learning insights. Also, for invaluable proof reading, thank you to Jamie, Mike, Pete, Joe, Mum and Dad.
...
There has been some improvement to adjust its fingerprint to match TLS as much as possible. However, this could not be done completely, so something more had to be done.
...
To circumvent this, users instead connect to private bridges in uncensored zones -- such as the United States -- so that the intended destination of the packets is hidden. However, with protocol fingerprinting by deep packet inspection (DPI) and the use of active probes, censors can still catch these unlisted bridges in under fifteen minutes of use!
...
What defines modern cryptography is the requirement to prove the hardness of breaking encryption, or at least compare it to the difficulty of another problem.
...
This is because these systems are based on mathematical equations linking the two key.
In many asymmetric schemes such as in the well known RSA cipher, the mathematics is based on group theory, the keys are definitions of groups and ciphertexts are made from elements inside them.
...
Honey encryption which was introduced in 2014 by Juels et al.~\cite{honey} has a similar tone to steganography, though instead of the resulting encoding appearing like innocuous data, it involves a ciphertext that can be decrypted to the original message or into an innocuous one.
...
\paragraph{ObfsProxy} simply encrypts the entire packet with a streaming cipher. Earlier versions used fixed parameters so although expensive, decryption could be done to obtain the original traffic but now even that is not possible since a key exchange produces ephemeral keys. This procedure produces ciphertexts that are computationally indistinguishable from a random string so intensive computational statistics would be able to differentiate this from a background distribution of standard traffic. Also, due to its complete lack of structure, it would not pass any protocol white-listing filters.
...
Formally, the scheme takes a formatting regular expression grammar $\mathcal{F}$ and lets $L(\mathcal{F})$ be the language defined by it. Then, using an existing authenticated encryption scheme $Enc_k$, the original message $M$ is encrypted and the unformatted ciphertext $Y$ is interpreted as an element of $\mathbb{Z}_{\vert L(\mathcal{F}) \vert}$. The encoding function $unrank :: \mathbb{Z}_{\vert L(\mathcal{F}) \vert} \rightarrow L(\mathcal{F})$ is then used to format it to specification. To make \texttt{unrank} and its inverse, decryption counterpart \texttt{rank} efficient the same precomputation is done as by Bellare and only a subset of the formatted language with a fixed length is used. This optimisation is key to be able to use this in practice. This turns the use of the scheme in to a block cipher used in Electronic Code Book mode. However, this lack of diffusion does not lead to pattern leakage since a random IV is used for each large block.
...
Although as a TOR proxy, the FTE scheme has had success in bypassing China's Golden Shield Project (GSP) censorship mechanisms and is listed as a official TOR transport.
...
\begin{mdframed}
\begin{verbatim}
GET /EopwqPHDbCy8WfNPEO3BPB.UUXEJ7xe2NoW/fSZQny4.x/jacDVkxjZQ4uSqgZu7.N2AGba
YeFqr/DEhbHSMhLjVf2cZy7z7iBGOhwsgtCegXuBNZn/VGXFLUmG55Hlk5bsMMhoO4tqF.mYApGx
Ad2c0GgoOnZLivQB.qTAzDFwxPtxS1i.bw2Oud2BQ7RtZaLoAcBJ40fjLwsLi7v18Oi9Q6cUqTqH
KTMsVWijB9/kh HTTP/1.1
\end{verbatim}
$\hookleftarrow$\\
$\hookleftarrow$
...
Although FTE produces validly formatted ciphertexts which bypass many current filtering systems, the scheme does not encapsulate any frequency information, so all possible ciphertexts are outputted uniformly and so look in no way normal.
...
Though, however you encode it, to fully define the probability density function for a bit stream of even a reasonable fixed length $n$, you need intractable exponential space.
...
Although, due to the central limit theorem, these measures could be used in our situation, the arbitrary structure of the distributions in question could result in false positives in the measurements if only the means of the samples were taken into consideration.

An exact test, such as Fisher's exact test, will calculate the exact probability of obtaining each observation in the sample.
...
$$G = 2 \Sigma_i O_i \cdot \ln{\frac{O_i}{E_i}} = 2N \Sigma_i o_i \cdot \ln{\frac{o_i}{e_i}} = 2N \cdot KL$$
...
I will discuss a method of calculating this threshold from positive and negative samples in section \ref{sec:thresholding}. However, this threshold can be adjusted by the censor to vary the false positive, true negative rate as applicable to their situation.
...
$Dec^{\mathcal{D}_2} :: ((k_e,k_s), C) \rightarrow M$, where $M$ is the original stream of messages that produced $C$.
...
I demand indistinguishability of ciphertexts under an adaptive chosen plaintext attack. Standard modes of authenticated encryption such as the cipher block chaining mode of AES with HMAC provide higher levels of security.
Though, since the normal traffic distribution is known publicly the produced ciphertexts are malleable, so chosen ciphertext attacks are always possible.
...
Firstly, I looked to the finite state machine produced by the regular expressions as used in FTE. Sadly, augmenting the nodes with weights according to the visiting frequency does not produce the desired effect.
...
However, these weights would not be meaningful, since they do not take into consideration the previous path through the transition graph and therefore previous symbols in the stream.
...
This makes detection easy for a censor, as these values are not allowed in the protocol, so they can be rejected without even considering their frequency of use in normal traffic.
...
However, the major downfall of this method is that the efficiency is greatly affected by the closeness of the two distributions, as the further they are apart the more samples that have to be rejected.
...
To test this, in early 2018, Horger et al. wrote a paper on training neural nets to generate samples from arbitrary distributions~\cite{deepl}.
...
Their model was able to produce results that have comparable goodness of fit to other classical methods and using an already trained network is competitive in performance.
However, large amounts of data are needed to train the system, where, if we desire practical performance, specialist hardware will be required to cope with the parallelism. Although, in the not so distant future this may become ubiquitous.
...
Therefore, I made an adaptation to make the system usable.
...
This new function is still surjective by roughly the same proof as before but its injectivity also holds.
Proof:
...
\paragraph{Reduction:}
Assume the existence of $\mathcal{A}$ to be the adversary that can distinguish ciphertexts of DE-CENSOR scheme $\mathcal{E}^\mathcal{D}$ with access to a plaintext oracle. Note: encode is a polynomial-time algorithm.
...
Though, note that statistical analysis techniques will be used to detect, but in practice, these are limited to only calculations that are computationally efficient.
...
In this experiment, I have fixed all other HTTP fields apart from the path to simplify the implementation. I have done this since this is only a proof of principle and expanding the scope to create more complexity is left as a future goal.
...
Since we can estimate the expected number of items to be used during the analysis, we can also pre-resize the hash table to avoid the expensive operation of resizing during operation to avoid collisions.
...
The server decodes and decrypts the data and follows the SOCKS protocol to forward the traffic on to its ultimate destination.
...
Firstly, I will test and explain how adjusting a heuristic parameter affects space requirements and ciphertext expansion.
...
It also discusses how this can be done at varying levels of granularity.
...
The figure on histogram-bin-numbers shows how, although each sample size does have diminishing returns, as pattern length increases, the maximum value reached does improve directly with the size of the sample. This is because, at small pattern sizes the samples are exhaustive, yet on larger patterns, although the actual normal distribution may contain up to an exponential number of sequences, in a fixed sample we only ever see a small portion of them.
...
\section{Statistical Indistinguishability}\label{sec:stat_ind}

To evaluate the success of the DE-CENSOR scheme I will experimentally demonstrate the closeness to which it resembles the target distribution.
To demonstrate this I recorded 3 reference distributions from normal traffic.
One was used to define the encoding scheme, $Enc^{\mathcal{D}_2}$ and $Dec^{\mathcal{D}_2}$; one was used to base the Censor Detection Scheme from; and one was used as a control set of background traffic.
I used 4 different statistical measurements and 10 samples of: FTE produced ciphertexts, DTE produced ciphertexts and normal traffic.
The results are outlined in the table.
...
Test					& FTE		& Normal	& DTE		& Result\\

\hline

Chi-square				& 9.0071	& 2.4376	& 0.2081	& Good\\

G-test					& 2.5116	& 0.7348	& 0.2425	& Good\\

Kullback-Leibler		& 1.2558	& 0.3674	& 0.1211	& Good\\

Jensen-Shannon			& 0.2292	& 0.0650	& 0.0339	& Good\\

\hline

\end{tabular}
\caption{Average statistical test results when compared to a normal reference distribution}
\end{table}

Firstly, the graphs in figure \ref{fig:stats} demonstrate the successful detection of the FTE ciphertexts, since a clear threshold would produce no false positives and true negatives. 
Secondly this shows each DTE score is consistent with, if not closer to the censor's reference distribution, than the normal traffic benchmarks.
Notice the the shape of the graphs is similar in figures b,c and d since the statistics have a similar derivation. 
In this case, there is no clear best statistical measure and so the Censor Detection Scheme could use any of the choices. 

\begin{figure}[h]
	\centering
	\begin{subfigure}[b]{0.4\linewidth}
		\includegraphics[width=\linewidth]{chi}
		\caption{Chi-square}
		\label{fig:chi}
	\end{subfigure}
	\begin{subfigure}[b]{0.4\linewidth}
		\includegraphics[width=\linewidth]{gtest}
		\caption{G-test}
		\label{fig:gtest}
	\end{subfigure}
	\begin{subfigure}[b]{0.4\linewidth}
	...
	\end{subfigure}
	\begin{subfigure}[b]{0.4\linewidth}
		\includegraphics[width=\linewidth]{js}
		\caption{Jensen-Shannon}
		\label{fig:js}
	\end{subfigure}

	\caption{Visualisation of the Statistic Measurements}\label{fig:stats}
\end{figure}
...
In this thesis, I started by explaining some of the benefits of a completely open web, gave examples where this is not the case, and outlined the negative impacts that this has; socially and economically.
I explained the focus of this thesis is on the type of censorship that blocks access to content, rather than removing it, and discussed the current ways this is implemented.
I described The Onion Router's part in the anonymisation of traffic and how the protocol based detection of its packets by network censoring agents is used to block it completely.
...
In this thesis, I have looked to the future, to try and pre-empt ways that could stop the current TOR transports from being effective.
...
I describe an efficient way to describe the intractable full definition of traffic distribution, using heuristics and give some promising examples.
I decide that pattern frequency is also a good distinguisher for FTE and, later, in the evaluation, conduct experiments to find an appropriate hyper-parameter value -- pattern length.
I then construct a detection mechanism using an appropriate test of similarities between distributions.
This calculates the distance measure between a test sample and a recorded background sample and concludes using a threshold whether the test sample is sufficiently close to `normal'.
Here, I leave the defining of the threshold up to the deploying-party, but as an example mention how one could be obtained.

To comprehensively define my new cryptographic primitive -- DTE -- I described the syntax and security properties.
I use a standard privacy property but also include a novel one called statistical indistinguishability. I formally define the requirement, but again leave the exact bound to the deploying party since there is no generic, asymptotic, security parameter and the exact value should be optimised for the situation.
...
I proposed an adaptation to ensure that the discrete version of the inverse transform  ($F^{-1}_{D_N}$) is invertible, and used it to form an encoding scheme.
I then defined DE-CENSOR to be the two stage encrypt-then-encode construction using a CBC encryption scheme and at least one level of encoding.
...
DE-CENSOR does not reach the requirement for statistical indistinguishability since the encryption scheme only produces ciphertexts that are indistinguishable from random. However, it cannot be distinguished efficiently. To prove this, I employed a hybrid argument, to join the indistinguishability property of the underlying encryption function and a reduction to the whole scheme.

\section{Project Goals}

During the thesis I have outlined the downfalls that I have found of using an FTE system to encode ciphertext -- the traffic it produces does not follow the same distribution as normal traffic and therefore can be detected.
I have prototyped the Censor Detection System in python, and tested it against normal and fake traffic. In section \ref{sec:stat_ind}, I show the detection systems success in producing no false positives or true negatives in the sample.
I have given detailed formalisation of the new cryptographic primitive -- DTE --- and discussed its use in replacing FTE.
Finally, not only have I constructed an instantiation of this scheme -- DE-CENSOR -- but I have also written a prototype proxy system that uses it and in experiments against the CDS in section \ref{sec:stat_ind}, I show how it has consistent scores with normal traffic across all example tests.
...
Distribution aware detection systems are only going to become more popular for conducting network based censorship. This thesis starts to provide a concrete ecosystem to manage how we reason about these systems and about how to defeat them.
