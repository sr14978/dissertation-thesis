...
I would like to take this opportunity to thank my supervisor -- Bogdan -- for his expertise and I would like to mention, Carl,x for his machine learning insights. Also, for invaluable proof reading, thank you to Jamie, Mike, Pete, Joe, Mum and Dad.
...
What defines modern cryptography is the requirement to prove the hardness breaking encryption, or at least compare it to the difficulty of another problem.
...
Although FTE produces validly formatted ciphertexts which bypass many current filtering systems, the ciphertexts look in no way normal.
...
Therefore, their suggestion was to instead concentrate on the distribution of values of certain heuristics which describe the packets.
...
\end{tabular}
\caption{Example statistical test results when compared to a normal reference distribution}
\end{table}

Firstly, this demonstrates the successful detection of the FTE ciphertexts, since a clear threshold would produce no false positives and true negatives. 
Secondly this shows each DTE score is consistent with, if not closer to the censor's reference distribution, than the normal traffic benchmark.
...
\begin{figure}[h]
	...
	\caption{Visualisation of the Statistic Measurements}\label{fig:hists}
\end{figure}
